...
<%@ attribute name="paging" type="com.dsup.chat.Paging"%>
...
<ul class="pagination">
	<li class="page-item"><a class="page-link"
		...
		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i">
			...
			<c:if test="${i == paging.page}">
				<li><a  class="page-link active">${i}</a>
			</c:if>
		</c:forEach>
	<li class="page-item"><a class="page-link"
		href="javascript:${jsFunc}(${paging.endPage<paging.lastPage?paging.endPage+1:paging.endPage})">다음</a>
</ul>