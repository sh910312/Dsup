...
public class SoketHandler extends TextWebSocketHandler implements WebSocketHandler {
	...
	public void sendMaseege(String cmd, String msg, String nickname, int chatid, String userId) throws IOException {
		...
		String temp = "<div class='pull-right'>" + date.format(new Date()) + "</div>" + "<h4>" + nickname + "</h4>"
				+ "<span> " + msg + "<input type='hidden' value='" + chatid + "'>" + "</span>";
		map.put("cmd", cmd);
		map.put("msg", temp);
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		int a = chatService.insertChat(cvo); // DB 저장
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		String msg = vo.getNickname() + "님이 퇴장하였습니다.";
		String nickname = "";
		int chatid = 0;
